...
internal class FeedMessageMapper : IFeedMessageMapper
{
    ...
    public IBetSettlement<T> MapBetSettlement<T>(bet_settlement message, IEnumerable<CultureInfo> cultures,
        byte[] rawMessage)
        where T : ISportEvent
    {
        if (message is null)
    ...
    public IRollbackBetSettlement<T> MapRollbackBetSettlement<T>(rollback_bet_settlement message,
        IEnumerable<CultureInfo> cultures, byte[] rawMessage)
        where T : ISportEvent
    {
        if (message is null)
            throw new ArgumentNullException($"{nameof(message)}");
    ...
    public IRollbackBetCancel<T> MapRollbackBetCancel<T>(rollback_bet_cancel message, IEnumerable<CultureInfo> cultures,
        byte[] rawMessage)
        where T : ISportEvent
    {
        if (message is null)
        ...
        where T : ISportEvent
    {
        ...
        return new BetCancel<T>(
            ...
            _producerManager.Get(message.product),
            (T)sportEvent,
            message.request_idSpecified ? (long?)message.request_id : default,
            message.start_timeSpecified ? message.start_time : default,
            message.end_timeSpecified ? message.end_time : default,
            message.superceded_by,
            message.market.Select(m => GetMarketWithCancel(m, sportEvent)),
            rawMessage);
    }

    public IFixtureChange<T> MapFixtureChange<T>(fixture_change message, IEnumerable<CultureInfo> cultures,
        byte[] rawMessage) where T : ISportEvent
    {
        if (message is null)
            throw new ArgumentNullException(nameof(message));
        ...
        return new FixtureChange<T>(
            ...
            (T)sportEvent,
            message.request_idSpecified ? message.request_id : null,
            message.change_typeSpecified ? message.change_type : default,
            message.next_live_timeSpecified ? message.next_live_time : null,
            message.start_time,
            rawMessage);
    }
        ...
        foreach (var keyValueString in splitSpecifiers)
        {
            ...
            if (splitKeyValue.Length != 2)
                throw new FormatException(
                    $"Incorrect format of the specifier {keyValueString}! It needs to consist a key and a value separated by \"=\"");

            var key = splitKeyValue[0].Trim();
            if (string.IsNullOrEmpty(key))
                throw new FormatException(
                    $"Incorrect format of the specifier {keyValueString}! The key must not be empty!");

            var value = splitKeyValue[1].Trim();
            if (string.IsNullOrEmpty(value))
                throw new FormatException(
                    $"Incorrect format of the specifier {keyValueString}! The value must not be empty!");

            if (result.ContainsKey(key))
                throw new ArgumentException(
                    $"There are two identical keys in specifiers: {specifiersString}, that should never happen!");
    ...
    private IMarketWithOdds GetMarketWithOdds(oddsChangeMarket oddsChangeMarket, ISportEvent sportEvent)
    {
        ...
        return new MarketWithOdds(
            ...
            oddsChangeMarket.outcome?.Select(outcome =>
                GetOutcomeOdds(outcome, oddsChangeMarket.id, specifiers, sportEvent)),
            sportEvent,
    ...
    private IMarketCancel GetMarketWithCancel(bet_cancel_market message, ISportEvent sportEvent)
    {
        ...
        return new MarketCancel(
            message.id,
            message.refid,
            GetSpecifiers(message.specifiers),
            message.extended_specifiers,
            message.groups?.Split("|"),
            _marketDescriptionFactory,
            sportEvent,

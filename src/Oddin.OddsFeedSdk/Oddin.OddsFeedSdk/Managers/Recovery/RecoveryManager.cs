...
namespace Oddin.OddsFeedSdk.Managers.Recovery
{
    ...
    internal class RecoveryManager : DispatcherBase, ISdkRecoveryManager
    {
        ...
        {
            var started = producerRecoveryData.LastRecoveryStartedAt ?? throw new SdkException("Inconsistent recovery state");
        ...
        {
            var eventRecovery = producerRecoveryData.GetEventRecovery(requestId) ?? throw new SdkException("Inconsistent event recovery state");
        ...
        private HttpStatusCode MakeEventRecovery(IProducer producer, URN eventId, Func<string, URN, long, int?, Task<HttpStatusCode>> apiCall)
        {
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    _log.LogInformation($"Sending recovery request (producer name: {producerName}, request ID: {requestId}, node ID: {nodeId})");
        ...
        ProducerRecoveryData FindOrMakeProducerRecoveryData(int producerId)
        {
            return _producerRecoveryData.GetOrAdd(producerId, l => new ProducerRecoveryData(l, _producerManager));
        }
    ...
    internal class ProducerRecoveryData
    {
        ...
        internal bool IsPerformingRecovery => RecoveryState is Recovery.RecoveryState.Started or Recovery.RecoveryState.Interrupted;
        ...
        internal void EventRecoveryCompleted(long recoveryId)
        {
        ...
        {
            return _currentRecovery?.RecoveryId == requestId || _eventRecoveries.ContainsKey(requestId);
        }
        ...
        internal void InterruptProducerRecovery()
        {
        ...
        {
            return messageInterest.MessageInterestType == MessageInterest.LiveMessagesOnly.MessageInterestType ||
                   messageInterest.MessageInterestType == MessageInterest.PrematchMessagesOnly.MessageInterestType;
        }

        private bool ValidateProducerSnapshotCompletes(ConcurrentDictionary<MessageInterest, bool> receivedSnapshotCompletes)
        {
            var producer = (Producer)_producerManager.Get(ProducerId);
    ...
    internal class EventRecovery : RecoveryData
    {
        ...
        internal EventRecovery(URN eventId, long recoveryId, long recoveryStartedAt) : base(recoveryId, recoveryStartedAt)
        {
            EventId = eventId;
        }
    }
